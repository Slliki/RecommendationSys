...
## 4. 多样性的提升（排序/召回）
### 1. 精排的多样性
- 精排阶段，结合兴趣得分和多样性分数对item排序
- 常用MMR,DPP等方法计算多样性得分
- 精排使用滑动窗口：保证窗口内的item相似度尽可能小（曝光页面中临近item相似度小）
- 粗排不使用滑动窗口：粗排需要保证整体多样性，且只是给精排提供候选集
- shuffle策略：
  - 类目：当前item之后k个位置内不允许出现和当前item相同二级类目的item
  - 多模态：提前计算item的多模态内容表征，并对全库item进行聚类（比如1000cluster），保证精排阶段当前item之后的k个位置内不允许出现和当前item相同cluster的item

### 2. 粗排的多样性
- 目标：给召回的结果（假设共5000个）进行打分，需要M个item（比如500个）送入精排
  - step1：对召回全集按得分`s_i`排序，**截取前k个（比如200个）直接送入精排**
  - step2：对剩余4800个item，每个item计算与已选中的200个item的差异，差异越大，多样性得分`d_i`越高
  - step3：结合`s_i`和`d_i`得到剩余4800个item的最终得分，选取300个送入精排

粗排的多样性提升类似精排，只是操作有区别。粗排结果直接选得分最高的部分送入精排，剩余的item需要根据与已送入精排的item计算多样性得分，结合兴趣得分后再次排序

### 3. 召回的多样性
#### 3.1 双塔模型
1. **添加噪声**：
   - baseline：用户塔将用户特征进行embedding，然后和物品塔特征进行检索相似度，召回最相似的物品
   - 线上召回过程可以在计算出用户向量后，**向用户向量添加随机噪声**（高斯噪声），然后再进行检索
   - 根据用户的兴趣宽窄（最近交互item的类目多则兴趣宽），决定噪声强度（**兴趣越窄越需要增加相似度，噪声越强**）
   - 理论上添加噪声会让召回结果不准导致指标下降，但是实际上增加多样性反而可以提升指标

2. **抽样用户行为序列**
   - baseline：用户塔的输入一般会concat用户行为序列特征（last-n item）
   - 线上召回过程可以对全体行为序列n进行抽样来提升多样性：
     - 保留最近的`r`个item(r<<n)
     - 从剩余`(n-r)`个item中随机抽样`t`个(t<<n)。可以均匀或非均匀抽样（非均匀可以保持类目平衡）
     - 总计得到`r+t`个item，作为用户行为序列的输入
   - 抽样用户序列可以**注入随机性**：每次召回都是使用不同的序列子集
   - 另一方面实际的lastN可能很大，如果每次都用全体N召回，我们使用的N可能无法很长。如果使用抽样就可以用更长的N进行子集选取，**利用更远的用户兴趣信息**，但是召回代价不增加

#### 3.2 U2I2I:抽样用户行为序列
U2I2I:user--item1--item2. item1是种子物品，是最近交互的n个物品之一
- n覆盖的类目数较少，且类目不平衡
- 做非均匀随机抽样，从n个物品选t个作为种子物品进行U2I2I. 
- 非均匀保证了类目平衡，提升多样性。同时抽样方法支持n的选取可以足够大，并保证代价不变


### 4. 探索流量多样性
- baseline：实际推荐系统会保留2%的流量作为**非个性化推荐**，进行兴趣探索
- 维护一些精选内容池，其中item交互率等指标应该高（根据人群划分，比如30-40岁男性用户内容池）
- 从精选池随机抽取几个item，直接跳过排序，插入到最终曝光结果。（只能强插或提权插入，因为这些物品一般不符合用户兴趣，会被排序模型淘汰）
- 短期这个方法可能会导致指标降低，但是长期是有利的，可以发掘用户潜在兴趣

## 5. 特殊人群的处理
- 新用户，低活用户行为少，个性化推荐不准确
- 新用户，低活用户更容易流失，要想办法提升留存（**基本只关注留存，不考虑其他消费指标**）
- 特殊用户的行为（点击率，交互率等）和主流用户不同，基于主流用户训练的模型在这时候的预测有偏

### 1.特殊的内容池
- 个性化推荐对特殊用户结果不好，需要特殊内容池保证质量
- 针对特定人群构造特定内容池，提升用户满意度（比如对中年女用户喜欢评论的特点，构造促进评论的内容池来吸引互动）
- **方法1**：**根据物品交互次数，交互率选择优质item**
  - 圈定人群：只考虑特定人群，比如使用18-25岁一二线城市男性
  - 构造内容池：用该人群对物品交互次数等进行打分，得分高的物品进入内容池
  - 弱个性化：这样的内容池有一定的弱个性效果，个性化来自于圈定的人群
  - 定期更新，加入新物品并排除交互率低和失去时效性的物品
  - 该内容池只对特定人群生效，对符合该池的人群推荐的时候，有少部分item来自该池
- **方法二：因果推断**
  - 判断物品对人群留存率的贡献，根据贡献值选取物品

特殊内容池一般用双塔模型进行召回。但是双塔模型基于个性化，而对新用户双塔模型本身个性化做不准，需要靠
特殊内容池的高质量/弱个性化内容弥补

额外训练代价：
- 对于正常用户，无论多少内容池都只需要训练并共用一个双塔模型分别从不同内容池召回
- 对于新用户，不可以用正常用户的双塔，需要单独训练

额外推理代价：
- 内容池定期更新，每次更新都需要同步更新向量数据库的的ANN（approximate nearest neighbor）索引
- 线上召回需要做ANN检索（少量算力）
- 特殊内容池一般不打，所以额外算力不多

### 2. 特殊的排序策略
1. 排除低质量物品
- 对特殊人群（新用户，低活），业务只关注留存，不关注消费（总曝光，广告收入等）
- 相应的策略是，对特殊人群少曝光广告（甚至可以不出广告）
- 新物品的冷启动避免在特殊人群中推荐，因为新物品的质量不确定，可能会导致流失。一般使用老用户探索新物品（损失较少）

2. 差异化融分公式
- 新用户，低活用户的交互行为少，人均点击量小，进一步交互可能低
- 在特殊人群的融分公式中提高点击率等权重（对比普通用户而言）
- 保留最终曝光坑位的几个位置给点击率预估最高的几个物品，其余坑位根据融分公式决定排序。这也是保证特殊用户点击率的方法

### 3. 特殊的排序模型
排序模型基于主流用户训练，对特殊用户的预测有偏。
- **方法1**：**大模型+小模型**
  - 用全量用户行为训练大模型，预估值`p`用于拟合用户行为`y`（y=1或0）
  - 用特殊人群行为训练小模型（可以用大模型相似结构，减少全连接层数得到）。
  - **小模型预估值`q`用于拟合大模型预估残差`y-p`，希望小模型纠正大模型的偏差**
  - 对主流用户只需要用大模型即可。**对特殊用户，用`p+q`作为最终预估值**

- **方法2：融合多个Experts**，类似MMoE
  - 只用一个模型，有多个experts，各自输出一个向量
  - 对experts结果加权平均，权重由一个小神经网络学习
  - 正常MMoED的权重神经网络输入是全体特征，这里**只使用用户特征计算权重向量**（用户活跃度，新/老用户）
  - 比如有两个experts，对应新老两种用户。如果一个用户是新用户，那么新用户expert权重为1.0，老用户expert权重为0.0

- **方法3：大模型预估后，小模型校准**
  - 大模型预估点击率，交互率
  - 将用户特征，大模型预估的点击率，交互率等作为小模型（如GBDT）的输入，再次预估点击率，交互率
  - 大模型用全量用户训练，小模型只用特殊用户，小模型用于拟合特殊用户的行为

## 6. 交互行为的改进
一般情况对交互行为的使用是：将模型预估的交互率指标用于融分公式，作为排序依据。可以尝试其他方法利用交互行为

### 1. 关注
关注量（用户关注作者的数量）对留存率有一定影响（正相关），关注量越高，一般留存率越高。但是关注量到达一定值后对留存影响开始减小。
对于关注量比较低的用户，提升关注量可以显著提升留存

- **方法1：排序策略提升关注量**
  - 对于用户u，模型预估后选物品i的关注率`p_i`
  - u已经关注了f个作者，定义单调递减函数`w(f)`，关注作者数越多，w越小
  - 在排序的融分公式添加`w(f)*p_i`，用于促进关注。f小且p_i大的时候，物品总评分提高
- **方法2：构造促关注内容池和召回通道**
  - 该内容池的物品关注率`p_i`高
  - 如果用户关注的作者数f小，则对该用户使用这个内容池
  - 召回配额可以固定，也可以根据f调整（f越小，配额越大）

UGC平台将作者侧指标（发布量，发布率）作为核心指标，希望作者发布更多内容
作者发布物品的交互效果可以提升作者积极性。当粉丝数少的时候，增加少量粉丝可以显著提升积极性（粉丝量大后边际效益递减）

- **方法3：排序策略帮助低粉用户涨粉：**
  - 某作者a的粉丝数`f_a`
  - 作者a发布的物品i可能被推荐给用户u，模型预估关注率`p_ui`（u看到i后关注作者a的概率）
  - 定义单调递减函数`w(f_a)`作为权重，f粉丝数越少，w越大
  - 排序融分公式原本一般都有`p_ui`,现在可以增加`w(f_a)*p_ui`项，提升低粉作者的推荐概率

### 2. 转发
A平台用户将物品转发到B平台，可以为A平台吸引站外流量。推荐系统做促转发（分享回流），可以提升DAU和消费指标

需要在不损害点击和其他交互行为的前提下，尽可能多吸引站外流量（促转发）

- 什么样的用户可以吸引大量站外流量？
  - **其他平台的KOL(Key Opinion Leader)**，即大V
  - 其他平台的KOL将本平台item转发到他的平台后容易引流到本平台
- 如何判断本平台用户是否是其他平台KOL？
  - 关注该用户的历史转发数据是否带来额外的站外流量

识别出KOL后的促转发策略（不损害其他指标的前提下）：
- **方法1：排序融分公式添加额外项`k_u*p_ui`**
  - `k_u`:当用户u是站外KOL时，`k_u`较大
  - `p_ui`:模型预估用户u看到物品i后转发的概率
  - 如果用户u是站外KOL，则多给他曝光他可能转发的物品。如果不是KOL的话`k_u`一般接近0
  - 保证该策略只影响少数特定用户
- **方法2：构造促转发内容池和召回通道，仅对站外KOL生效**

### 3. 评论
评论的目标是促进发布，提高作者积极性。如果新发布物品尚未获得很多评论，可以提权预估评论率，促进获得评论

- **方法1：排序融分公式添加额外项`w_i*p_i`**
  - `w_i`：权重，与物品i已获得的评论数负相关
  - `p_i`：模型预估用户看到物品i后评论的概率
- **方法2：构造促评论内容池和召回通道**
  - 一些用户喜欢留评论，和作者或其余用户互动，针对这些用户构造促评论内容池
  - 一些用户留的评论质量高（点赞多），鼓励这些用户多评论来提升留存